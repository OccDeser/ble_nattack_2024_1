...
\begin{table}[ht]
    \centering
    \caption{Results of WPA2-PSK } \label{tab:4-way-handshake-result}
...
We performed a study of the session establishment protocol of BC by analyzing several scenarios under the QA and SA attack models and the results of our analysis are summarized in \autoref{tab:bc-session-result}.
...
\begin{table*}[t]
    \centering
    \caption{RESULTS OF BC SESSION ESTABLISHMENT} \label{tab:bc-session-result}
    \renewcommand{\arraystretch}{1.2} \setlength{\tabcolsep}{8pt}
    \begin{threeparttable}[b]
...
We demonstrated the effectiveness of our proposed methodology through case studies on WPA2-PSK, BLE pairing protocols, and BC session establishment. 
Our methodology successfully rediscovered known attacks such as the Kracks attack on WPA2 and the KNOB attack on BLE and BC.
Additionally, we discovered a novel Unilateral Multi-Downgrade (UMD) attack on BC session establishment that previous models failed to detect. 
This is because previous work did not model general brute-force attacks, which our approach addresses, enabling the identification of these overlooked security threats.
