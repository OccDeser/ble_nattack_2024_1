...
\begin{enumerate}[leftmargin=*]
    ...
    Additional mechanisms are needed to describe a key's entropy in order to cover brute-force
    attacks within the symbolic model.
    ...
    Capturing these prerequisite conditions within a symbolic model,
    without tying them to one protocol, presents a significant challenge,
    as these conditions vary across different protocols and keys.
...
This paper aims to bridge the existing gap by introducing a protocol-independent and automated
method for modeling low-entropy keys and the adversarial brute-force capabilities within symbolic
analysis frameworks.
We focus on enhancing \Tamarin{} to describe brute-force attacks without encoding them separately
for each protocol.
...
This tool supports an adversary model that extends beyond the Dolev-Yao model,
capable of capturing brute-force behaviors that a deterministic attacker can carry out by
recomputing a function forward and comparing the result against an observed output,
including attacks in which the attacker combines several such recoveries across sessions.
Our approach can be used to identify security issues arising from low-entropy keys in protocols
and to check protocols that tolerate low-entropy keys for attacks within a stated attacker model
and budget.
...
\begin{itemize}[leftmargin=*]
    ...
    \item \emph{Tool without kernel modification.} We developed EntropyVerif, a syntax-sugar extension of \Tamarin{}, in which users annotate low-entropy terms with \texttt{:= L}.
          \emph{The compiler emits standard multiset-rewriting rules and leaves the \Tamarin{}
          backend unmodified, so every proof is carried out by \Tamarin{} itself.}
