...
This subsection reports three things: what our translation costs on protocols that contain a
low-entropy value but no reachable brute-force attack, how the annotated models compare against
the same models without annotations, and where the additional proof effort goes.

\noindent\textbf{Version attribution.}
The WPA2-PSK, BLE and BC results were produced with \Tamarin{}~1.8.0 and Maude~2.7.1.
The Alipay models below use syntax that \Tamarin{}~1.8.0 does not accept, so they were run with
\Tamarin{}~1.10.0 and Maude~3.1.
Each pair of numbers we compare comes from one version on one machine, and we never place numbers
from the two versions in the same comparison.
...
The change to each model is one label marking the password as low entropy. For the order-code
model we also report a variant in which one line is added to the existing tactic, in both the
annotated and the unannotated version.
In all four, every lemma reaches the same verdict as in the unannotated model and the finished
proofs have the same number of steps, so the analysis outcome does not change.
\autoref{tab:perf-baseline} gives the running times. For the first two models EntropyVerif adds
about twelve seconds. The order-code model is far more expensive in both versions, and the
one-line tactic adjustment brings them to 42\% and 32\% of the unadjusted times, which
indicates that its long runtime comes from a suboptimal tactic in the original model rather than
from the annotation.

\begin{table}[t]
    \centering
    \caption{Verification time of the Alipay models with and without low-entropy annotations, under \Tamarin{}~1.10.0.}
