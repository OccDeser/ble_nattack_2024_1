...
In this section, we will discuss the limitations of existing models and introduce our methodology
for capturing brute force behaviors without encoding them separately for each protocol.
...
The recovery is deterministic and forward: for each candidate value of $k$ the attacker recomputes
$f$ and keeps the candidate whose output matches the observed one exactly.
It never applies an inverse of $f$ to the observed output, and checks a candidate by recomputing
$f$ rather than by inverting it.
We call an attack that reaches its goal through one or more such recoveries a \emph{forward
brute-force attack}, and this is the class of attacks we capture.
...
To capture forward brute-force attacks, we designed a key-breaking oracle.
This oracle accepts any function symbol declared in the model and returns the value of the
low-entropy key, provided that the attacker can furnish the function output along with the values
of all other arguments, as shown below:
\begin{figure}[h]
    \centering
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Uncertainty of Received Functions:} The oracle is not written for one protocol,
    so it has to accept any function symbol the model declares.
...
\noindent
Consider a function $f$ of arity $n+1$ whose output $f(x_1,\ldots,x_n,k)$ the attacker has
observed, where $k$ is the target key and $x_1,\ldots,x_n$ are the remaining arguments.
Suppose the attacker learns that argument $x_i$ has the value $a_i$ and submits that value
together with the observed output to the relationship-breaking oracle.
The oracle returns the partial application of $f$ in which the $i$-th argument is fixed to $a_i$:
\begin{flalign*}
    & f'_{a_i}(x_1,\ldots,x_{i-1},x_{i+1},\ldots,x_n,k) &\\
    & \quad = f(x_1,\ldots,x_{i-1},a_i,x_{i+1},\ldots,x_n,k). &
\end{flalign*}
The subscript is part of the definition. Fixing the same position to a different value yields a
different function, so $f'$ is determined by $f$, the position $i$, and the value $a_i$ together.
Repeating this removes known arguments one at a time until $k$ is the only argument left, at which
point the attacker calls the value-breaking oracle on the remaining output.
...
The relationship-breaking oracle maintains a collection of all function symbols declared in the
model.
