...
\begin{enumerate}[leftmargin=*]
    ...
    Brute-force attacks involve an attacker repeatedly attempting keys, contingent on prior knowledge of the algorithm, its outputs, and other relevant inputs beside targeted keys~(e.g., salt values, initialization vectors). 
    Accurately and generally capturing these prerequisite conditions within a symbolic model presents a significant challenge, as these conditions vary across different protocols and the specific keys targeted.
    \item \textbf{Enhanced Adversarial Capabilities:}
    In existing works, an attacker is typically limited to comprising specific keys. However, in practice, an attacker may attempt to brute-force any key as long as its entropy does not exceed their computational capacity. 
    The attacker may also deconstruct complex keys into simpler sub-keys and brute-force them individually. 
    A key challenge lies in how to model this flexible brute-forcing capability within a symbolic model.
\end{enumerate}

This paper aims to bridge this gap by introducing a general and automated method for modeling low-entropy keys and the adversarial brute-force capabilities within symbolic analysis frameworks. 
We focus on enhancing the TAMARIN prover to describe general brute-force attacks conducted by adversaries. 
TAMARIN is a state-of-the-art protocol verification tool widely used for formal analysis of many protocols, including TLS\cite{cremers2017tls13}, 5G-AKA\cite{basin20185G, wang20215G}, EMV\cite{basin2021emv,radu2022emv}, etc.
We have designed a new syntax, which is a superset of the syntax of TAMARIN, allowing for precise annotation of low-entropy variables in symbolic models. 
Meanwhile, we have developed an automated protocol analysis tool supporting such low-entropy annotated models, which we call EntropyVerif\cite{toolrepo}. 
This tool supports an adversary model that goes beyond the Dolev-Yao model, capable of capturing all brute-force behaviors against security protocols, including complex attacks where an attacker synthesizes and utilizes various attack capabilities.
...
\begin{itemize}[leftmargin=*]
    \item  We loosen the \emph{perfect encryption assumption} in the symbolic model. 
    Our methodology's adversary model surpasses the Dolev-Yao adversary. 
    ...
    \item  We applied our methodology to develop a protocol analysis tool that accepts models with low-entropy annotations. This tool can automatically analyze vulnerabilities associated with low-entropy keys in protocols and generally capture brute-force attacks.
    \item Using our tool to analyze real-world protocols such as WPA2-PSK, BLE pairing protocol and BC session establishment protocol, we discovered several known low-entropy associated vulnerabilities in these protocols, like the PMKID attack and KNOB attacks.
    ...
    \item The UMD attack significantly weakens the strength of the session keys in BC devices. Even when the longest keys are used, an attacker can repeatedly downgrade one side of the two devices to obtain different segments of the session key and ultimately reconstruct the complete key.
