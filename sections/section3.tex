...
\section{Modeling of General Brute Force Attacks} \label{sec:3symbolic-modeling}
...
Attackers can obtain the value of $s$ by providing all other inputs of $f4$ and its output.
This rule is highly intuitive and checks the three essential prerequisites we have summarized for a brute-force attack: knowledge of the function, knowledge of all inputs except the key, and knowledge of the computation result. 
It can effectively capture brute-force attacks targeting the $f4$ function.
...
\noindent
To model this oracle, three are two challenges.
\begin{enumerate}
    \item \textbf{Uncertainty of the received function:} As a general oracle, it should accept any function. 
...
\noindent An attacker submits to the value-breaking oracle the output of a function that uses a low-entropy key as its sole input.
