...
\begin{table}[t]
    ...
    Symbol & Meaning \\
    \midrule
    $\delta(t)$                 & Sub-terms that can be deconstructed from $t$ \\
    $\rho(t)$                   & Atomic terms required to construct $t$ \\
    $c(T)$                      & Terms in $T$ composed only of constants \\
    $\tau(F)$                   & Terms extracted from a fact sequence $F$ \\
    $\varepsilon(T)$            & Deconstruction of every term in $T$ \\
...
To aid in the description, we define the following operations.
...
For instance, $[F_{L}] \rightarrow [F_{R}]$ denotes a rule where $F_{L}$ and $F_{R}$ are sequences of facts.
...
For a given rule $[F_L] \rightarrow [F_R]$, $T_{Low}$ denotes the set of low-entropy terms labeled in the rule, where $T_{Low} \subseteq \varepsilon(\tau(F_L)) \circ \varepsilon(\tau(F_R))$.
The translated version of such a rule with low-entropy labels is shown as follows:
\[
    [F_L] \rightarrow [F_R \circ \text{\textsf{LE}}(T_{Low})]
\]
...
Given the rule $[F_L] \rightarrow [F_R]$, $\tau(F_R)$ denotes the terms present within the rule after executing, whereas $\varepsilon(\tau(F_L))$ represents all possibly accessible terms before the execution.
...
\[
    [F_L] \rightarrow [F_R \circ \text{\textsf{MD}}(\tau(F_R) \setminus \varepsilon(\tau(F_L)))]
\]
