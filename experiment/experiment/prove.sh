...
# MODELFILE="./experiment/entropy_models/$MODEL"
# export LC_ALL=C.UTF-8 && \
#     tamarin-prover +RTS -N8 -RTS --derivcheck-timeout=0 $MODELFILE \
#     --prove --stop-on-trace=SEQDFS --output=$OUTFILE > $OUTFILE.tmp \
#     && echo "" >> $OUTFILE && cat $OUTFILE.tmp >> $OUTFILE && rm $OUTFILE.tmp
...
MODEL="wpa2_four_way_handshake_unpatched.spthy"
OUTFILE="$OUTDIR/original/$MODEL"
MODELFILE="./experiment/entropy_models/$MODEL"
export LC_ALL=C.UTF-8 && \
    tamarin-prover +RTS -N6 -RTS --derivcheck-timeout=0 $MODELFILE \
    --prove=krack_attack_ptk --output=$OUTFILE > $OUTFILE.tmp \
    && echo "" >> $OUTFILE && cat $OUTFILE.tmp >> $OUTFILE && rm $OUTFILE.tmp