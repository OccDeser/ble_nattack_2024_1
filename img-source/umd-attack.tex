...
\draw[ARROW] (c_init_sc) -- (c_atta_sc) node[midway,above=-0.3ex]{$\text{SC}_\text{I}$: True};

\coordinate (c_atta_sc) at ($(c_atta_sc)+(0,-1ex)$);
\coordinate (c_res_sc) at ($(c_atta_sc-|n_responder)$);
\draw[ARROW] (c_atta_sc) -- (c_res_sc) node[midway,above=-0.3ex]{$\text{SC}_\text{I}$: \textcolor{xhs_red}{False}};

\coordinate (c_res_sc) at ($(c_res_sc)+(0,-3.2ex)$);
\coordinate (c_atta_sc) at ($(c_res_sc-|n_attacker)$);
\draw[ARROW] (c_res_sc) -- (c_atta_sc) node[midway,above=-0.3ex]{$\text{SC}_\text{R}$: True};

\coordinate (c_atta_sc) at ($(c_atta_sc)+(0,-1ex)$);
\coordinate (c_init_sc) at ($(c_atta_sc-|n_initiator)$);
\draw[ARROW] (c_atta_sc) -- (c_init_sc) node[midway,above=-0.3ex]{$\text{SC}_\text{R}$: \textcolor{xhs_red}{False}};
